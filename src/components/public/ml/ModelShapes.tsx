...
export type ShapeKind = 'line' | 'cuts' | 'forest';

const MARKS: Record<ShapeKind, React.ReactNode> = {
  line: <line x1={at(4)} y1={at(78)} x2={at(96)} y2={at(14)} stroke="var(--accent-500)" strokeWidth="2.5" strokeLinecap="round" />,
  cuts: (
    <>
...
/** One panel on its own. The comparison table puts one at the top of each
 *  column, so the picture and the words about it arrive together. */
export function ModelShape({ kind }: { kind: ShapeKind }) {
  return (
...
function Panel({ caption, kind }: { caption: string; kind: ShapeKind }) {
  return (
    <div>
      <ModelShape kind={kind} />
      <p
        style={{
          marginTop: 9,
...
export default function ModelShapes({ captions }: { captions: [string, string, string] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 10 }}>
      <Panel caption={captions[0]} kind="line" />
      <Panel caption={captions[1]} kind="cuts" />
      <Panel caption={captions[2]} kind="forest" />
    </div>
  );
}
